...
colt_ollama_model = 'llama2-uncensored'
colt_vision_model = 'gemma3'
...
def build_system_prompt(user_name, user_nickname):
    ...
    system_prompt = f"""Here are your rules: {colt_rules}
...
# === Main Entry Point ===
async def COLT_Message(message_author_name, message_author_nickname, message_content, attachment_url, attachments):
    llm_response = None
    ...
    if attachment_url:
        ...
        if image_file_name:
            llm_response = await COLT_Converse_Image(
...
async def COLT_Converse_Image(user_name, user_nickname, user_input, image_file_name, attachments, attachment_url):
    # check who we are currently talking too - if someone new is talking to us, fetch their memories
    # if it's a different user, cache the current history to file the swap out the memories
    await switch_current_user_speaking_too(user_name, user_input)
...
async def switch_current_user_speaking_too(user_name, user_response):
    ...
    cached = colt_current_session_chat_cache.get(user_name)
    # print(cached)
    if cached:
...
def update_conversation_history(user_name, new_messages):
    ...
    colt_current_session_chat_cache[user_name] = json.dumps(current_user_conversation_messages)
